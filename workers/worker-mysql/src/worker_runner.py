...
from worker_mysql.engine import execute, server_version
from worker_mysql.registry import WORKER_NAME, build_worker_info, register_engines

DB_STARTUP_TIMEOUT_S = int(os.environ.get("MYSQL_STARTUP_TIMEOUT_S", 180))
...
def wait_for_server() -> str:
    ...
    while time.time() < deadline:
        try:
            ...
            time.sleep(2)
    raise RuntimeError(f"MySQL did not become available in {DB_STARTUP_TIMEOUT_S}s: {last_error}")
...
def main() -> None:
    ...
    print(f"[Worker] Connected to MySQL {version}.", flush=True)
