...
MatchResult* timeout_wrapper(const char *text_cstr, const char *pattern_cstr, int timeout_ms){
     std::future<MatchResult*> future = std::async(std::launch::async, find_matches_internal, text_cstr, pattern_cstr);
...
extern "C" MatchResult *find_matches(const char *text_cstr, const char *pattern_cstr) {
    int timeout_ms = 5000;
    return timeout_wrapper(text_cstr, pattern_cstr,timeout_ms );
}